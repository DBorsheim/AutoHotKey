...
global F2Preset := "Zoom Transform"
global F3Preset := "Blank Lumetri"
global F4Preset := "IOKTBS Lumetri Color Preset"
global F5Preset := "Looks Preset"
global F6Preset := "Drop Shadow"
global F7Preset := "Zoom Transform"
global F8Preset := "Zoom Transform"
global F9Preset := "Zoom Transform"

PleasantNotify("Premeire Hotkeys", "F2: " . F2Preset . "`rF3: " . F3Preset . "`rF4: " . F4Preset . "`rF5: " . F5Preset . "`rF6: " . F6Preset . "`rF7: " . F7Preset . "`rF8: " . F8Preset . "`rF9: " . F9Preset . "`rF10: Replace File", 500, 330, "vc hc", "2")
...
Escape::ExitApp
F2::
	PleasantNotify("Premeire Hotkeys", "F2: " . F2Preset, 500, 100, "vc hc", "1")
	preset(F2Preset)
	return
F3::
	PleasantNotify("Premeire Hotkeys", "F3: " . F3Preset, 500, 100, "vc hc", "1")
	preset(F3Preset)
	return
F4::
	PleasantNotify("Premeire Hotkeys", "F4: " . F4Preset, 500, 100, "vc hc", "1")
	preset(F4Preset)
	return
F5::
	PleasantNotify("Premeire Hotkeys", "F5: " . F5Preset, 500, 100, "vc hc", "1")
	preset(F5Preset)
	return
F6::
	PleasantNotify("Premeire Hotkeys", "F6: " . F6Preset, 500, 100, "vc hc", "1")
	preset(F6Preset)
	return
F7::
	PleasantNotify("Premeire Hotkeys", "F7: " . F7Preset, 500, 100, "vc hc", "1")
	preset(F7Preset)
	return
F8::
	PleasantNotify("Premeire Hotkeys", "F8: " . F8Preset, 500, 100, "vc hc", "1")
	preset(F8Preset)
	return
F9::
	PleasantNotify("Premeire Hotkeys", "F9: " . F9Preset, 500, 100, "vc hc", "1")
	preset(F9Preset)
	return
F10::
	PleasantNotify("Premeire Hotkeys", "Replace File", 500, 100, "vc hc", "1")
	MouseClick, right
	sleep 200
	send, {Up}
	sleep 5
...
^Space:: presetType("test")




;Duplicate Sequence, Iterate, Open new Sequence, close old One
;Duplicate sequence, ctrl + D
;Iterate, enter to edit title, copy, take text string, remove last 8 chars, look at last two chars as numbers from string, increase the count, keep leading zero, remove last two chars, concatenate number at the end, put it in as the clipboard, paste it back In
;Open new sequecne, ctrl + alt + 0
;Close old sequence, might not be necessary anymore
...
PleasantNotify(title, message, pnW=700, pnH=300, position="b r", time=10) {
	...
	Gui, Notify: Font, c0xE2E2E2 s18 wBold, Segoe UI
	Gui, Notify: Add, Text, % " x" 20 " y" 12 " w" pnW " vpn_title", % title
	Gui, Notify: Font, c0xE2E2E2 s15 wRegular
	Gui, Notify: Add, Text, % " x" 20 " y" 56 " w" pnW " h" pnH " vpn_msg", % message
	RealW := pnW + 50
	RealH := pnH + 20
	Gui, Notify: Show, W%RealW% H%RealH% NoActivate
	WinMove(PN_hwnd, position)
	WinSet, Region,-0-0 w%pnW% h%pnH% R20-20,%A_ScriptName%

	winfade("ahk_id " PN_hwnd,210,20)
	if (time <> "P")
	{
		Closetick := time*1000
...
ifWinNotActive ahk_exe Adobe Premiere Pro.exe ;the exe is more reliable than the class, since it will work even if you're not on the primary Premiere window.
	{
	goto theEnding2 ;and this line is here just in case the function is called while not inside premiere. In my case, this is because of my secondary keyboards, which aren't usually using #ifwinactive in addition to #if getKeyState(whatever). Don't worry about it.
	}
;;---------You do not need the stuff ABOVE this line.--------------
